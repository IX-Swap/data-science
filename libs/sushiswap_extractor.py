...
def get_pool_history(contract_id: str, range_limit: int=100) -> list:
    ...
    for skip in tqdm(range(0, range_limit)):
        try:
            ...
            # get current response and extract from it last timestamp, extend swaps history with current response
            response = client.execute(query)
...
def get_pool_burns(contract_id: str, range_limit: int) -> list:
...
def pool_reserves_to_df(reserves_list: list) -> pd.DataFrame:
    ...
    daily_data_df['volumeUSD'] = daily_data_df['volumeUSD'].astype('float')
    daily_data_df['reserveUSD'] = daily_data_df['reserveUSD'].astype('float')
