...
class Simulation:
    ...
    def run_simulation(self, isBurnAvailable: bool, isMintAvailable: bool):
        ...
        for vm in [False, True]:
            ...
            self.normalize_csv(f'{base_experiment_path}/{self.iteration}/swaps.csv',
                               ['token_in_amount', 'token_out_amount', 'token_out_amount_min', 'system_fee',
                                'oracle_amount_out'],
    ...
    def read_simulation(self, sim_id: int):
        ...
        swaps_df.loc[:, 'price_diff'] = (swaps_df['reserve_Y'] / swaps_df['reserve_X'] - swaps_df['reserve_Y_before'] /
                                         swaps_df['reserve_X_before']) / (
    ...
    def show_mitigation_comparison(self, pure_df: pd.DataFrame, mitigated_df: pd.DataFrame):
        ...
        print(
            f"TWAP unavailability ratio for {self.x_name}/{self.y_name} is {mitigated_df[mitigated_df.mitigator_check_status == 'CANT_CONSULT_ORACLE'].shape[0] / mitigated_df.shape[0]}")
    ...
    def plot_slice_distribution_and_diff_limit_line(self, mitigated_df: pd.DataFrame):
        ...
        ax.set_title(
            f'Swaps slice_factor/out_amount_diff scatterplot for {self.x_name}/{self.y_name} pool (split by status)')
    ...
    def plot_cumulative_prices_with_and_without_mitigation(self, pure_df: pd.DataFrame, mitigated_df: pd.DataFrame,
        ...
        ax[0].plot(pure_df.transaction_timestamp,
                   pure_df.price_X_cumulative if not make_big_num_convert else pure_df.price_X_cumulative.to_numpy().astype(
                       np.float64),
                   label=f'reserve_X ({self.x_name})')
        ax2.plot(pure_df.transaction_timestamp,
                 pure_df.price_Y_cumulative if not make_big_num_convert else pure_df.price_Y_cumulative.to_numpy().astype(
                     np.float64),
        ...
        ax[1].plot(mitigated_df.transaction_timestamp,
                   mitigated_df.price_X_cumulative if not make_big_num_convert else mitigated_df.price_X_cumulative.to_numpy().astype(
                       np.float64),
                   label=f'reserve_X ({self.x_name})')
        ax2.plot(mitigated_df.transaction_timestamp,
                 mitigated_df.price_Y_cumulative if not make_big_num_convert else mitigated_df.price_Y_cumulative.to_numpy().astype(
                     np.float64),
                 label=f'reserve_Y ({self.y_name})', color='orange')
    ...
    def plot_transactions_by_type(self, mitigated_df: pd.DataFrame, ignore_success: bool = False,
        ...
        if separate_plots:
            ...
            ax[0].set_title(
                f'Transaction values for {self.x_name} side of {self.x_name}/{self.y_name} pool respective to statuses (Volatility mitigator on)')
            ax[1].set_title(
                f'Transaction values for {self.y_name} side of {self.x_name}/{self.y_name} pool respective to statuses (Volatility mitigator on)')
            ...
            ax.set_title(
                f'Transaction values for {self.x_name}/{self.y_name} pool respective to statuses (Volatility mitigator on)')
    ...
    def plot_price_distribution(self, pure_df: pd.DataFrame, mitigated_df: pd.DataFrame, width: int = 15,
                                height: int = 5, separate_plots: bool = False):
        ...
        if separate_plots:
            ...
            ax[0].plot_date(x=pure_df['transaction_timestamp'], y=pure_df['X_price'], linestyle='-', marker=None,
                            label=f'{self.x_name} price (mitigation off)', color='red')
            ax[0].plot_date(x=mitigated_df['transaction_timestamp'], y=mitigated_df['X_price'], linestyle='--',
                            marker=None, label=f'{self.x_name} price (mitigation on)', color='maroon')
            ax[1].plot_date(x=pure_df['transaction_timestamp'],
                            y=(np.ones(len(pure_df['X_price'])) / pure_df['X_price']) if (
                                    pure_df['X_price'] != 0).all() else np.zeros(len(pure_df['X_price'])),
            ...
            ax[1].plot_date(x=mitigated_df['transaction_timestamp'],
                            y=(np.ones(len(mitigated_df['X_price'])) / mitigated_df['X_price']) if (
                                    mitigated_df['X_price'] != 0).all() else np.zeros(len(mitigated_df['X_price'])),
            ...
            ax.plot_date(x=pure_df['transaction_timestamp'], y=pure_df['X_price'], linestyle='-', marker=None,
                         label=f'{self.x_name} price (mitigation off)', color='red')
            ax.plot_date(x=mitigated_df['transaction_timestamp'], y=mitigated_df['X_price'], linestyle='--',
                         marker=None, label=f'{self.x_name} price (mitigation on)', color='maroon')
    ...
    def plot_frequency_distribution(self, swaps_df: pd.DataFrame, width: int = 15, height: int = 5):
        ...
        plt.plot_date(resampled24.index, resampled24.values, markersize=1, linestyle='solid', marker='None',
                      label='Nr. transactions per day')
        plt.plot_date(mov_avg.index, mov_avg.values, markersize=1, linestyle='solid', marker='None',
    ...
    def plot_price_impact(self, pure_df: pd.DataFrame, mitigated_df: pd.DataFrame, width: int = 15, height: int = 5,
                          smallest_y: float = 0, biggest_y: float = 0):
        ...
        ax.plot_date(data=pure_df, x='transaction_timestamp', y='price_diff', linestyle='solid', color='red',
                     marker=None, label='Mitigation off')
        ax.plot_date(data=mitigated_df, x='transaction_timestamp', y='price_diff', linestyle='solid', marker=None,
                     label='Mitigation off')
    ...
    def extract_filtered_and_mevs_dfs(self):
    ...
    def extract_suspicious_and_filtered_swaps_dfs(self, difference_threshold_percents: int = 5):
        ...
        for name, group in grouped:
            ...
            # print(fr"perc1 = {perc_diff0}; and perc2 = {perc_diff1}; {s0.txd}")
            # print(fr"perc1 was received by {out1} and {in0}; and perc2 by {out0} and {in1}")
            # print(fr"{abs(out1 - in0)} and {math.ceil(out1 + in0)}")
            # print(fr"{abs(out0 - in1)} and {math.ceil(out0 + in1)}")
    ...
    def show_swaps_and_mevs_by_token(self, swaps_df: pd.DataFrame, mevs_df: pd.DataFrame, width: int = 10,
                                     height: int = 10):
        fig, ax = plt.subplots(figsize=(width, height))
    ...
    def show_swaps_and_mevs_daily_count_by_token(self, swaps_df: pd.DataFrame, mevs_df: pd.DataFrame,
                                                 mevs_alter_axis: bool = False, width: int = 15, height: int = 7):
        ...
        if not mevs_alter_axis:
            fig, ax = plt.subplots(figsize=(width, height))
            daily_mevs_df = mevs_df['timestamp'].dt.floor('d').value_counts().rename_axis('date').reset_index(
                name='count')
            daily_mevs_df = daily_mevs_df.sort_values(by='date')
            ax.hist(swaps_df['timestamp'],
                    bins=(swaps_df.iloc[len(swaps_df) - 1]['timestamp'] - swaps_df.iloc[0]['timestamp']).days,
    ...
    def show_mevs_to_swaps_ratio(self, swaps_df: pd.DataFrame, mevs_df: pd.DataFrame, width: int = 10, height: int = 5):
        ...
        daily_swaps_df = swaps_df['timestamp'].dt.floor('d').value_counts().rename_axis('date').reset_index(
            name='count')
        ...
        ax2.plot_date(daily_swaps_mevs_df['date'], daily_swaps_mevs_df['count'], color='b', linestyle='--',
                      label='Swaps count')
        ax.set_xlabel('Time')
    ...
    def show_mevs_to_reserves_ratio(self, swaps_mitigation_off_df: pd.DataFrame, mevs_df: pd.DataFrame, width: int = 10,
                                    height: int = 10):
        ...
        """
        daily_mevs_count_df = mevs_df['timestamp'].dt.floor('d').value_counts().rename_axis('date').reset_index(
            name='mevs count')
        daily_reserves_avg_df = swaps_mitigation_off_df.groupby(
            swaps_mitigation_off_df['transaction_timestamp'].dt.floor('d')).mean()
        daily_reserves_avg_df = daily_reserves_avg_df.rename_axis('date')
        daily_mevs_count_and_reserves_avg_df = pd.merge(daily_mevs_count_df,
                                                        daily_reserves_avg_df[['reserve_X', 'reserve_Y']], on='date')
    ...
    def show_mevs_values_to_reserves_ratio(self, swaps_mitigation_off_df: pd.DataFrame, mevs_df: pd.DataFrame,
        ...
        daily_mevs_amount_usd_avg_df = daily_mevs_amount_usd_avg_df.rename_axis('date').reset_index(
            name='amount_usd_avg')
        daily_reserves_avg_df = swaps_mitigation_off_df.groupby(
            swaps_mitigation_off_df['transaction_timestamp'].dt.floor('d')).mean()
        daily_reserves_avg_df = daily_reserves_avg_df.rename_axis('date')
        daily_mevs_count_and_reserves_avg_df = pd.merge(daily_mevs_amount_usd_avg_df,
                                                        daily_reserves_avg_df[['reserve_X', 'reserve_Y']], on='date')
        daily_mevs_count_and_reserves_avg_df = daily_mevs_count_and_reserves_avg_df.sort_values(by='date')
        ...
        ax2.plot(daily_mevs_count_and_reserves_avg_df.date,
                 daily_mevs_count_and_reserves_avg_df[
                     'amount_usd_avg'] / daily_mevs_count_and_reserves_avg_df.reserve_Y,
    ...
    def get_original_swaps_df(self) -> pd.DataFrame:
        return pd.read_pickle(
            fr'{os.getcwd()}\data\pair_history\{self.x_name}_{self.y_name}\{self.x_name.lower()}_{self.y_name.lower()}_swaps.pkl')

    def get_reserves_df(self) -> pd.DataFrame:
        return pd.read_pickle(
            fr'{os.getcwd()}\data\pair_history\{self.x_name}_{self.y_name}\{self.x_name.lower()}_{self.y_name.lower()}_reserves.pkl')

    def get_daily_token_price(self, first_to_second: bool, token_name: str) -> pd.DataFrame:
        """
        get daily token price based on reserves
        :param first_to_second: perform price estimation of first column to second or not
        :param token_name: name of the token for which price is calculated
        :return: dataframe of the target token price distribution to paired one
        """
        reserves_df = pd.read_pickle(
            fr'{os.getcwd()}\data\pair_history\{self.x_name}_{self.y_name}\{self.x_name.lower()}_{self.y_name.lower()}_reserves.pkl')
        reserves_df[token_name + '_price'] = reserves_df.apply(lambda x: x.reserve0 / x.reserve1,
                                                               axis=1) if first_to_second else reserves_df.apply(
            lambda x: x.reserve1 / x.reserve0, axis=1)
    ...
    def calculate_attack_profit_by_token(self, mevs_df: pd.DataFrame) -> pd.DataFrame:
        mevs_df['first_token_value'] = mevs_df.apply(
            lambda x: x.amount_in if x.token_in < x.token_out else x.amount_out, axis=1)
        mevs_df['second_token_value'] = mevs_df.apply(
            lambda x: x.amount_out if x.token_in < x.token_out else x.amount_in, axis=1)

        first_token_grouped_df = mevs_df.groupby(by=['timestamp', 'sender']).first_token_value.agg(
            [lambda x: x.max() - x.min()])
        second_token_grouped_df = mevs_df.groupby(by=['timestamp', 'sender']).second_token_value.agg(
            [lambda x: x.max() - x.min()])

        grouped_mevs_df = pd.merge(mevs_df, first_token_grouped_df, on=["timestamp", "sender"], how='left')
        grouped_mevs_df = grouped_mevs_df.rename(columns={'<lambda>': str(str(
            grouped_mevs_df.iloc[0].token_in if grouped_mevs_df.iloc[0].token_in < grouped_mevs_df.iloc[
                0].token_out else grouped_mevs_df.iloc[0].token_out) + '_profit')})

        grouped_mevs_df = pd.merge(grouped_mevs_df, second_token_grouped_df, on=["timestamp", "sender"], how='left')
        grouped_mevs_df = grouped_mevs_df.rename(columns={'<lambda>': str(str(
            grouped_mevs_df.iloc[0].token_out if grouped_mevs_df.iloc[0].token_in < grouped_mevs_df.iloc[
                0].token_out else grouped_mevs_df.iloc[0].token_in) + '_profit')})
