...
def main(): 
    ...
    for x in [5500, 30000, 75000, 150000, 350000, 750000, 5000000]:
        ...
        #    transactions_history_path = f'{BASE_DIR}/{iteration}/transaction_history.csv'
            ...
            for subindex, vm in enumerate([False, True]):
                ...
                for timestamp, cummulative_freq, scale_deviation, token_in, token_out in transactions:
                    ...
                    amount_in = scipy.stats.weibull_min.ppf(cummulative_freq, shape, loc=0, scale=scale)## c freq
                    amount_in = expand_to_18_decimals(amount_in)

                    if timestamp - start_time <= timedelta(days=1):
                        amount_in = amount_in // 1000 
                
                    amm.swap(cnt, Transaction(timestamp, amount_in, token_in, token_out, 100))
                    cnt += 1
...
def simulate_transactions(start_time):
    ...
    Y_transactions = list(zip(timestamps[Y_NAME], cummulative_frequencies[Y_NAME], scale_deviations_list[Y_NAME], [Y_NAME] * len(timestamps[Y_NAME]), [Y_NAME] * len(timestamps[Y_NAME])))
