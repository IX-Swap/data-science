...
def main(): 
   ...
   # INITIAL_RESERVES_USD = 10000
    ...
    for scale, shape, initial_reserves_usd in grid_dist_params:
        ...
        simulate_transactions(start_time, OCCURENCES_PER_MIN*2, X_NAME, Y_NAME, transactions_history1_path, shape, DIST_LOC, scale)
        simulate_transactions(start_time, OCCURENCES_PER_MIN, Y_NAME, X_NAME, transactions_history2_path, shape, DIST_LOC, scale*2)
        ...
        for subindex, vm in enumerate([False, True]):
            ...
            save_config(f'{BASE_DIR}/{iteration}/{subindex}/config.txt', OCCURENCES_PER_MIN, initial_reserves_usd, INITIAL_SEC_PRICE, scale, shape, PRICE_TOLLERANCE_THRESHOLD, WINDOW_SIZE, GRANULARITY, vm)
...
def simulate_transactions(start_time, mean_occurencies_per_min, token0, token1, transaction_history_filename, shape, loc, scale):
    ...
    simulation_seconds_total = 60*24*15
