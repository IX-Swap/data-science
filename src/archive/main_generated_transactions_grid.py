...
from trading_simulation import CauchyGenerator, MonteCarloTransactionSimulator, PoissonGenerator, Transaction
import blockchain
...
def main(): 
    ...
    initial_reserves_usd_list = [50000, 100000, 500000]
    ...
    BASE_DIR = f'../data/simulated_transactions_grid/experiment_{EXPERIMENT_ID}'
    ...
    for mean_occurencies_per_min, initial_reserves_usd, ratios_sec_usd, cauchy_scale, price_tollerance_threshold, window_size, granularity, vm in grid:
        ...
        save_config(f'{BASE_DIR}/{iteration}/config.txt', mean_occurencies_per_min, initial_reserves_usd, ratios_sec_usd, cauchy_scale, price_tollerance_threshold, window_size, granularity, vm)

        transactions_history1_path = f'{BASE_DIR}/{iteration}/history1.csv'
        transactions_history2_path = f'{BASE_DIR}/{iteration}/history2.csv'

        simulate_transactions(start_time, mean_occurencies_per_min, X_NAME, Y_NAME, transactions_history1_path, cauchy_scale, ratios_sec_usd)
        simulate_transactions(start_time, mean_occurencies_per_min, Y_NAME, X_NAME, transactions_history2_path, cauchy_scale, ratios_sec_usd)
        ...
        amm.reset(X_NAME, Y_NAME, initial_reserves_usd // ratios_sec_usd , initial_reserves_usd, vm, window_size * 60 * 60, window_size * 60 * 60// granularity, granularity) #todo: beautify
...
def simulate_transactions(start_time, mean_occurencies_per_min, token0, token1, transaction_history_filename, cauchy_scale, token_in_ratio):
    simulator = MonteCarloTransactionSimulator(
        ...
        CauchyGenerator(loc=0, scale=cauchy_scale, limit=500000), token0, token1,
    )
