...
def simulate_transactions(current_iteration_timestamp, initial_reserve, initial_sec_price, scale_list, shape_list, transaction_history_path):
    ...
    params['Y']['scale'] = scale_list[reserve_range_index]
    params['X']['scale'] = scale_list[reserve_range_index] / initial_sec_price
    ...
    for action in actions:
        if action[0] == 'PASS':
            ...
            for _ in range(cycles):
                ...
                prices.append(reserve_X / reserve_Y)
                
                #if reserve_range_index != prev_reserve_range_index:
                #    price_scale = reserve_X / reserve_Y

                #prev_reserve_range_index = reserve_range_index

                if len(prices) < 100:
                    price_scale = 1
                else:
                    price_scale = sum(prices[-50:-20]) / len(prices[-50:-20])

                params['Y']['shape'] = params['X']['shape'] = shape
                params['Y']['scale'] = scale
                params['X']['scale'] = scale * price_scale
                ...
                x_swaps_simulator.token_in_generator.scale = params['X']['scale']
                ...
                y_swaps_simulator.token_in_generator.scale = params['Y']['scale']
